...
func ServeInternal(logger *logrus.Logger) error {
	...
	healthChecker.AddLiveness("ping", health.HTTPGetCheck(fmt.Sprint("http://", viper.GetString("internal.address"), "/ping"), time.Minute))
...
func ServeExternal(logger *logrus.Logger) error {
	...
	dbSQL, err := sql.Open("postgres", viper.GetString("database.dsn"))
	if err != nil {
		return err
	}
	defer dbSQL.Close()
	...
	if err != nil {
		logger.Errorf("Error opening DB %s", viper.GetString("database.dsn"))
		return err
	}
	defer db.Close()
	...
	s, err := server.NewServer(
		...
		server.WithGateway(
			...
			gateway.WithEndpointRegistration(viper.GetString("gateway.endpoint"), pb.RegisterTestServerHandlerFromEndpoint, pb.RegisterIpsHandlerFromEndpoint),
		),
...
func setDBConnectionTest() {
	viper.Set("database.dsn",
		"host=localhost port=35000 user=postgres password=postgres sslmode=disable dbname=testserver")
}
