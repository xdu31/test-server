...
//
// TODO: Extend the TestServer service by defining new RPCs and
...
//
// TODO: Create an implementation of the TestServer server
...
// implementation of TestServer server interface service, but only
...
// Default implementation of the TestServer server interface
...
func NewBasicServer(database *gorm.DB) (pb.TestServerServer, error) {
	return &server{db: database}, nil
}

func NewIpsServer(database *gorm.DB) (pb.IpsServer, error) {
	return &ipsServer{&pb.IpsDefaultServer{DB: database}}, nil
}

type ipsServer struct {
	*pb.IpsDefaultServer
}
