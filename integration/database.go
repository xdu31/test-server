...
// ResetDB drop tables and does auto migration
func (cfg PostgresDBConfig) ResetDB() {
	// wait 5s for DB docker to be up
	time.Sleep(time.Second * 5)
	db, err := sql.Open("postgres", cfg.GetDSN())
	if err != nil {
		log.Fatalf("unable to connect to %s database: %v", cfg.DBName, err)
	}
	defer db.Close()
	...
	if _, err := db.Exec(resetQuery); err != nil {
		log.Fatalf("unable to drop tables in %s database: %v", cfg.DBName, err)
	}
	// run migrations if a migration function has exists
	if cfg.MigrateFunction != nil {
		if err := cfg.MigrateFunction(*db); err != nil {
			log.Fatalf("unable to migrate %s database: %v", cfg.DBName, err)
		}
	}
}
