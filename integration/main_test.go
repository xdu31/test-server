...
import (
	...
	migrate "github.com/xdu31/test-server/db"
)
...
func TestMain(m *testing.M) {
	...
	if db, err := NewTestPostgresDB("testserver", migrate.MigrateDB); err != nil {
		log.Fatalf("unable to create test testserver database: %v", err)
	} else {
		dbTest = *db
	}
	...
	closeDatabase, err := dbTest.RunAsDockerContainer("test-server-application-test")
	if err != nil {
	...
	closeServer, err := RunBinary("server", "--database.dsn", dbTest.GetDSN())
	if err != nil {
